...
def oco_day_files(oco_dir, years):
    oco_files = []
    for year in years:
        year_str = str(year)[2:4]
        files_for_year = glob.glob(f'{oco_dir}/oco2_LtCO2_{year_str}*.nc4')
...
def oco_gridding(oco_file, grid):
    ...
    overpass_date = oco_df['time'].dt.date.iloc[-1]
    ...
    oco_df['time'] = pd.to_datetime(oco_df['time'])
    oco_df['rounded_time'] = oco_df['time'].dt.floor('D')
